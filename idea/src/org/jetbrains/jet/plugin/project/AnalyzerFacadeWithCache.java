...
public final class AnalyzerFacadeWithCache {
    ...
    private static final SLRUCache<JetFile, CachedValue<ResolveSessionResult>> RESOLVE_SESSION_CACHE = new SLRUCache<JetFile, CachedValue<ResolveSessionResult>>(1, 3) {
        @NotNull
        @Override
        public CachedValue<ResolveSessionResult> createValue(final JetFile file) {
            final Project fileProject = file.getProject();
            return CachedValuesManager.getManager(fileProject).createCachedValue(
                    new CachedValueProvider<ResolveSessionResult>() {
                        @Nullable
                        @Override
                        public Result<ResolveSessionResult> compute() {
                            Project project = file.getProject();
                            ...
                            return Result.create(new ResolveSessionResult(file, resolveSession), PsiModificationTracker.MODIFICATION_COUNT);
                        }
                    },
                    true);
        }
    ...
    @NotNull
    public static ResolveSessionResult getLazyResolveSession(@NotNull JetFile file) {
